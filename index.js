...
const { joinVoiceChannel, getVoiceConnection, entersState, VoiceConnectionStatus, VoiceConnectionDisconnectReason, EndBehaviorType, createAudioPlayer, createAudioResource, StreamType, NoSubscriberBehavior, AudioPlayerStatus } = require('@discordjs/voice');
...
async function joinChannelIfNeeded(channel) {
  ...
  if (!me) {
    ...
    if (!hasLoggedVoicePermissions) {
      console.log(`Bot voice permissions in ${channel.name}: ${perms.toArray().sort().join(', ')}`);
      hasLoggedVoicePermissions = true;
    }
    if (missing.length > 0) {
      console.warn(`Missing voice permissions in ${channel.name}: ${missing.join(', ')}`);
      return;
    }
  }
  ...
  console.log(`[voice] Joining ${channel.name} in ${guildName} — waiting for Ready...`);

  connection.on('error', (err) => {
    console.error(`[voice] Error in ${guildName}:`, err.message ?? err);
  });

  connection.on('debug', (msg) => {
    console.log(`[voice:debug] ${guildName}: ${msg}`);
  });

  // Count how many times the connection has cycled back to Signalling without
  // ever reaching Ready.  The library auto-calls configureNetworking() each
  // time it receives a fresh VOICE_SERVER_UPDATE, so we must NOT call it
  // ourselves — that creates a second Networking instance that races the first.
  let signallingCycles = 0;
  let readyOnce = false;

  connection.on('stateChange', async (oldState, newState) => {
    console.log(`[voice] ${guildName}: ${oldState.status} → ${newState.status}`);

    // ── Ready ──────────────────────────────────────────────────────────────
    if (newState.status === VoiceConnectionStatus.Ready) {
      signallingCycles = 0;
      voiceReconnectAttempts.delete(guildId);
      voiceReadyLocks.delete(guildId);
      if (!readyOnce) {
        readyOnce = true;
        console.log(`[voice] READY in ${guildName}`);
    ...
    if (newState.status === VoiceConnectionStatus.Destroyed) {
      console.log(`[voice] DESTROYED in ${guildName}`);
      voiceConnections.delete(guildId);
      voiceReceiverReady.delete(guildId);
      voiceReadyLocks.delete(guildId);
      cleanupVoiceKeepAlive(guildId);
      return;
    }

    // ── Disconnected — try rejoin with exponential backoff ─────────────────
    if (newState.status === VoiceConnectionStatus.Disconnected) {
      if (
        newState.reason === VoiceConnectionDisconnectReason.WebSocketClose &&
        newState.closeCode === 4014
      ) {
        console.warn(`[voice] WS close 4014 in ${guildName} (kicked/moved) — waiting for reconnect...`);
        try {
      ...
      if (attempt <= 5) {
        ...
        console.log(`[voice] Disconnected in ${guildName} — rejoin attempt ${attempt}/5 in ${delay}ms...`);
        await new Promise(r => setTimeout(r, delay));
        try {
          connection.rejoin();
        } catch (err) {
          console.warn(`[voice] rejoin() failed in ${guildName}:`, err.message);
          if (connection.state.status !== VoiceConnectionStatus.Destroyed) connection.destroy();
        }
      } else {
        console.warn(`[voice] Max rejoin attempts reached in ${guildName}. Giving up.`);
        if (connection.state.status !== VoiceConnectionStatus.Destroyed) connection.destroy();
      }
      return;
    }

    // ── Signalling — the library is negotiating a new voice server session ──
    // Each Signalling→Connecting cycle is one automatic retry by the library.
    // We give it up to 10 cycles before concluding the connection is hopeless.
    if (newState.status === VoiceConnectionStatus.Signalling) {
      signallingCycles++;
      console.log(`[voice] ${guildName}: signalling cycle #${signallingCycles}`);
      if (signallingCycles > 10) {
        console.warn(`[voice] ${guildName}: too many signalling cycles — destroying connection.`);
        connection.destroy();
      }
    }
  });
...
function clearVoiceReadyCheck(guildId) {
  ...
  if (timeout) { clearTimeout(timeout); voiceReadyTimeouts.delete(guildId); }
}
